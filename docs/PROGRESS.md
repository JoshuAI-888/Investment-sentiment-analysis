...
| F21 | MCP server and MCP Apps surface | 3 | `merged 2026-09-05` | Hand-rolled JSON-RPC 2.0 dispatcher at `POST /api/mcp`, `requireUser()`-gated. 8 named tools + one generated per registered metric, all through one shared `ToolResultEnvelope`. Rule 1 (no raw corpora, ever) proven by a corpus-leak test written first. All three `ui://` resources render `n`/window/coverage-floor/disclosure in markup. Catalogue generated from `analytics/registry.ts`, registry-drift tested. 1488/1488 unit on its own tree, 1555/1555 on the full merged tree; contract 148/148; integration 428/428; build clean. **Deferred:** `test:e2e` — no Playwright browsers available in this sandbox (network-blocked download), same gap F10/F11 reported; spec written and covers the DoD's full e2e row. Two disclosed interpretive gaps: F20's per-item evidence fields carry no `calculationId` (not `CalculationArtifact`s, matching F09's own precedent); `market.spike_detection` still isn't in `analytics/registry.ts`, so `explain_spike` names this in its own `limitations[]`. See `progress/log/2026-09-05-f21-mcp-surface.md` |

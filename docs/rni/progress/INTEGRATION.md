...
| `apps/web/migrations/0024_rni_universe_upgrade.sql`, cited-synthesis and model-budget PostgreSQL tests | Implement D-RNI-26 canonical verification stage, reason-bound skipped plans and candidate/selection separation | `CURRENT` | focused PostgreSQL cited publication 24/24 and model budget 15/15; typecheck/lint/diff pass |
| `apps/web/src/rni/config/{index,rights-policy}.ts` | Expose D-RNI-26's server-owned frozen P0 rights authority for trusted adapter injection | `CURRENT` | focused unit 1/1; typecheck/lint/diff pass |
...
| `CURRENT` | Implement D-RNI-26 cited-publication schema compatibility | PostgreSQL cited publication 24/24 and model budget 15/15; typecheck; focused lint; diff check |
| `CURRENT` | Expose the server-owned P0 rights-policy authority | unit 1/1; typecheck; focused lint; diff check |

...
| ER-15 | P1 | `OPEN` | Corrected task parsers are strict, but literal dynamic-input closing sentinels remain unescaped inside model-visible source/company text and can spoof the evidence boundary | Use an unspoofable length-prefixed/encoded envelope or escape sentinel characters, then prove embedded delimiters yield exactly one effective boundary for structured and discovery calls |
| ER-16 | P1 | `OPEN` | Version lookup exists, but reconstructed discovery/verification v1 definitions and the current discovery serializer do not replay the originally accepted prompt bytes | Preserve prompt, schema and serializer snapshots byte-for-byte per `(task, promptVersion)` and compare historical requests to the accepted v1 fixtures |
| ER-17 | P1 | `OPEN` | Start/finalize and governed limits now exist, but post-response model/tool/schema failures discard available response ID, token, latency and cost telemetry | Retain the sanitized parsed provider envelope in failed finalization and test drift, forbidden-tool and invalid-output failures so billed attempts remain auditable for D-RNI-21 |
| ER-18 | P1 | `OPEN` | E05 persists an insertion-order classifier input hash while E09 recursively sorts the dispatched JSON, so observation and invocation hashes disagree for the same model call | Export one canonical serializer/hash for classifier construction and routing, and prove the persisted observation hash equals the recorded exact dispatched-input hash |

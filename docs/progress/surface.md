...
| F16b | Scheduler admin plane | 4 | `merged 2026-09-05` | — | **Wave 4 half of F16**: admin-editable job rows (due times, cadence, enabled, retry policy, per-job budget ceiling) through F15's mutation pipeline; zero-external-call dry run proven against a real dispatch handler; next-run preview (reuses F16a's `computeNextDueAt`, DST-correct). ADR-013 grep-verified: no code path can rewrite the QStash schedule or `vercel.json`. Additive extension to `repositories/jobs.ts` (`findJobDefinitionById`/`listJobDefinitions`/`updateJobDefinition`, optimistic concurrency on `(id, version)`) — same authorized cross-lane pattern as F16a's own gap-fills. Coordinator-verified independently: lint/typecheck clean, unit 1426/1426 on the full merged tree, contract 114/114, integration 402/402 (incl. a genuinely concurrent dispatcher-vs-admin-edit race test), build clean. Scoping disclosed: `maxCallsPerRun` not exposed (F18's territory); dry run authorized/audited directly rather than through the mutation pipeline (writes no state for it to apply to). See `progress/log/2026-09-05-f16b-scheduler-admin-plane.md` |
| F17 | Architecture Explorer | 5 | `merged 2026-09-05` | — | `/architecture`: versioned topology manifest (imports `providerId`/job-key constants/`MODEL_TASKS`/`SCORER_IDS` directly, no hand-copied value — grep-verified), allowlist-only public-safe projection (proven by construction with hostile-input contract tests), formula examples that run the production calc library and persist/link one real `CalculationArtifact` per registered method (all 20), the searchable `/architecture/calculations` catalogue, full keyboard/reduced-motion/static-alternative accessibility, CI manifest reconciliation failing on drift in either direction. Opportunities tab names real gaps rather than hiding them (Reddit-unwired/D-39, missing Substack collector service, X-sampling stub, the config_version bootstrap gap, missing scorer-identity commit-SHA query path). Coordinator-verified independently: lint/typecheck clean, unit 1492/1492 on the full merged tree, contract 120/120, integration 385/385, build clean. Full e2e run by the build agent (134/135 — the one failure is a pre-existing, unrelated F08 flake, reproduced independently on a clean checkout with zero F17 changes, filed as its own follow-up) but not independently re-run by the coordinator given its wall time. See `progress/log/2026-09-05-f17-architecture-explorer.md` |
| F18 | Cost, budgets, degradation | 5 | `merged 2026-09-05` | — | Real `BudgetPolicy`/`BudgetDecision`, decimal-safe, pre-dispatch global enforcement at D-32's actual thresholds ($290/$320/$350, not the spec's stale pre-D-20 literal $80/$90/$100 — matching F15's own seeded settings). Per-account budgets correctly absent (void, D-11). Full degraded-state catalogue against the current 9-provider roster (no Reddit row, D-39). `x-budget.ts` now carries real D-32 X-read ceilings. `pnpm test:chaos` (7 cases). **A real design error was caught by the build agent's own self-review, not the coordinator**: ApeWisdom was first classified as reduce-tier-gated "optional," which the agent found would risk silent D-16 corpus loss for D-39's now-only attention collector — reverted to permissive/critical before merge. Coordinator-verified independently: lint/typecheck clean, unit 1539/1539 on the full merged tree, contract 130/130, integration 415/415, chaos 7/7, build clean. Deferred/disclosed: chaos coverage for only 2 of 9 providers (the two with a real fetcher seam); no persisted circuit-breaker-state table for a live `/admin/data-sources` indicator (contract request to SPINE); no LLM budget-hook point without editing `services/llm/` (out of lane, reported not worked around). See `progress/log/2026-09-05-f18-cost-budgets-degradation.md` |

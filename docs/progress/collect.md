...
| F16a | Dispatch core and trigger path | 1 | `blocked` | — | **Wave 1 half of F16** (`../features/F16-scheduler-dispatcher.md` §0): signature verification, Redis lock, idempotency, `JobService`, the price-trigger path, the heartbeat. **No UI** — job definitions are seeded rows and changing one is a migration. Blocked on **MT-04**. **SPINE's jobs repository merged 2026-09-04** (PR #12, 4 review rounds) — the `JobService` persistence this feature needs now exists, so MT-04 (create the QStash schedule) is the only remaining blocker, and the app now has a stable production URL (`investment-sentiment-analysis.vercel.app`) to point the schedule at |
...
| F16a | **MT-04** — QStash schedule not created | Owner creates it once, now that the app has a stable production URL. **SPINE's jobs repository is merged, so this is the only remaining blocker** |

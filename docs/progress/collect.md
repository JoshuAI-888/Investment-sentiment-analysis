...
| F16a | Dispatch core and trigger path | 1 | `not started` | — | **Wave 1 half of F16** (`../features/F16-scheduler-dispatcher.md` §0): signature verification, Redis lock, idempotency, `JobService`, the price-trigger path, the heartbeat. **No UI** — job definitions are seeded rows and changing one is a migration. **Unblocked 2026-09-05 — MT-04 done.** The QStash schedule is created and firing against production; `repositories/jobs.ts` (SPINE's gap-fill, PR #12) already exists. This is now the genuinely next COLLECT pickup |
...
| ~~F16a~~ | ~~MT-04~~ | **Resolved 2026-09-05.** The owner confirmed the QStash schedule is created and firing. F16a is this lane's next pickup |

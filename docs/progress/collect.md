...
| F16a | Dispatch core and trigger path | 1 | `unblocked 2026-09-05` | — | **Wave 1 half of F16** (`../features/F16-scheduler-dispatcher.md` §0): signature verification, Redis lock, idempotency, `JobService`, the price-trigger path, the heartbeat. **No UI** — job definitions are seeded rows and changing one is a migration. **MT-04 closed 2026-09-05** (owner-confirmed: the QStash schedule exists and points at production), so this feature has **no remaining blocker** — it is buildable now, and it is the single gate on every collection path. **SPINE's jobs repository merged 2026-09-04** (PR #12, 4 review rounds), so the `JobService` persistence this feature needs already exists, and the schedule points at the stable production URL (`investment-sentiment-analysis.vercel.app`) |

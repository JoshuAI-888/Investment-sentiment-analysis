...
import { env } from '@/env';
import { rniAiRouteSettingUpdateRequest, rniErrorEnvelope } from '@/rni/contracts';
      ...
      : error instanceof UnauthorizedError
        ...
        : error instanceof RniAiRouteSettingsError && error.kind === 'invalid'
          ? ([
              400,
              'INVALID_REQUEST',
              'Provide a valid route, reason and Idempotency-Key.',
              false,
            ] as const)
          : error instanceof RniAiRouteSettingsError && error.kind === 'conflict'
            ...
            : ([
                ...
                'AI route configuration is currently unavailable.',
                true,
              ] as const);
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(
      { data: await createLiveAiRouteSettingsService(session.userId).getCurrentAiRouteSetting() },
      { headers },
    );
  } catch (error) {
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const parsed = rniAiRouteSettingUpdateRequest.omit({ idempotencyKey: true }).safeParse(body);
    if (!parsed.success) throw new RniAiRouteSettingsError('invalid');
    const data = await createLiveAiRouteSettingsService(session.userId).updateFutureAiRoute({
      ...parsed.data,
      idempotencyKey: key,
    });

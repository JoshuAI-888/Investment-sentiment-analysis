...
describe('F12 eval harness — integration against the frozen starter corpus', () => {
  it('the Tier C gate passes when every reference answer is well-judged', async () => {
    ...
    expect(verdict.overallMean).toBeGreaterThanOrEqual(4.0);
  ...
  it('verifier metrics (B7/B8) are computed from the seeded-error corpus', () => {
    ...
    expect(measurement.goodCount).toBe(seeded.length);
    // This toy verifier only catches a couple of the eight fault classes by design (it is not
  ...
  it('runEvalHarness assembles a full run record and formatEvalReport prints a per-axis table and the Tier B/C verdicts', async () => {
    const judged = await judgeCorpus();
